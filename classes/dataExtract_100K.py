...
class extractor:
    ...
    # def __init__(self, data_json_path, save_type, save_dir, last_slice_tid):
        ...
        self.dataset_type = {1:'raw dataset', 2:'testset & trainset', 3:'challenge set'}
    ...
    def test_train_set(self):
        ...
        df_test_p = df_seed_25more.sample(n=50, replace=False, random_state=1)
        ...
        for i in pid2pnt_group.keys(): 
            ...
            print("* playlist_track记录数（测试集中含", i, "首歌曲）: ", df_chosen.shape[0])
        ...
        print("* playlist_track记录数（测试集）: ", self.df_testset.shape[0]) 
        ...
        df_test_p = df_seed_100more.sample(n=20, replace=False, random_state=1)
        pid2pnt = list(set(df_test_p['pid'].tolist()))
        ...
        print('* playlist_track记录数（测试集中含100首歌曲）:', df_chosen.shape[0])
        self.df_testset = self.df_testset.append(df_chosen)
        self.df_trueset = self.df_trueset.append(df_true)
        print("* playlist_track记录数（测试集）: ", self.df_testset.shape[0])
        ...
        df_test_p = df_rest.sample(n=30, replace=False, random_state=1)
        pid2pnt = list(set(df_test_p['pid'].tolist()))
        test_pid.extend(pid2pnt)
        ...
        pid2pnt_group = {'zero':pid2pnt[0:10], 'nt':pid2pnt[10:]}
        ...
        print("* playlist_track记录数（测试集中没有标题的）: ", df_chosen.shape[0])
        self.df_testset = self.df_testset.append(df_chosen)
        print("* playlist_track记录数（测试集）: ", self.df_testset.shape[0])
